...
def generic_dataset():
    return {
        ...
        "rows": [{"row": {"text_input": "test prompt"}}],
    }
...
def test_base64_images(generic_dataset):
    ...
    prompt = updated_generic_dataset["rows"][0]["row"]["text_input"]
    ...
    assert images[0]["image_url"].startswith(f"data:image/png;{upload_method},")
