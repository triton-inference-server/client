...
class ModelClient(BaseModelClient):
    ...
    def get_lib(self):
        """Returns tritonclient library for given scheme."""
        return {"grpc": tritonclient.hl.lw.grpc, "http": tritonclient.hl.lw.http}[self._triton_url.scheme.lower()]
            ...
            while True:
                ...
                if isinstance(item, Exception):
                    if hasattr(item, "message"):
                        ...
                        message = f"Error occurred during inference request. Message: {item}"
                    _LOGGER.error(message)
