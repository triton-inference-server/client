...
class ModelConfigParser:
    ...
    def from_dict(cls, model_config_dict: Dict) -> TritonModelConfig:
        ...
        """
        LOGGER.debug(
            f"Parsing Triton config model from dict: \n{json.dumps(model_config_dict, indent=4)}"
        )
        ...
        if isinstance(backend_parameters_config, list):
            ...
            LOGGER.debug(
                f"backend_parameters_config is a list of strings: {backend_parameters_config}"
            )
            ...
            LOGGER.debug(
                f"backend_parameters_config is a dictionary: {backend_parameters_config}"
            )
            backend_parameters = {
