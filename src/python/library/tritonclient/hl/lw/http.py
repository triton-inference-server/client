...
class InferenceServerClient():
    ...
    def stream_request(self, model_name, model_version, inputs_for_stream):
        ...
        try:
            with requests.post(request_url, json=inputs_for_stream, headers=headers, stream=True) as response:
                if response.status_code != 200:
                    _LOGGER.debug(f"Request failed with status code: {response}")
                    self._exception = Exception(f"Request failed with status code: {response.status_code}")
                    self._event.set()
                    return
                ...
                try:
                    for line in response.iter_lines():
                       if line:  # Filter out keep-alive new lines
                            ...
                            response = InferResponse(outputs_struct, parameters={"triton_final_response": False})
            ...
            response = InferResponse(outputs=None, parameters={"triton_final_response": True})
            self.process_chunk(response, None)
        except Exception as e:
            _LOGGER.debug(f"Some error occurred while processing the response: {e}")
            self._exception = e
    ...
    def async_stream_infer(self, model_name, model_version, inputs, outputs, request_id, enable_empty_final_response, **kwargs):
        """Perform inference."""
        model_version = model_version if model_version else "1"
        inputs_for_stream = {input_value.name(): _array_to_first_element(input_value._np_data) for input_value in inputs}

        self._event.clear()
        self._exception = None
        self._stream_thread = threading.Thread(target=self.stream_request, args=(model_name, model_version, inputs_for_stream))
